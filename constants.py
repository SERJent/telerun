...
invulnerability_t = 500  # Время неуязвимости после потери жизни
t_vul = 0  # Счетчик который отвечает за учет invulnerability_t в процедуре check_lives()
t_add = 20 # Для счетчика , можно изменять в зависимости от того, сколько должен самолет быть не уязвимым
rescue_spd = 2 * spd
